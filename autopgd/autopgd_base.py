...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None):   
        ...
        #x_adv = x_adv.clamp(0., 1.) # ONLY FOR IMAGE DATA
        x_best = x_adv.clone()
        ...
        for i in range(self.n_iter):
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    topk_curr = torch.clamp((1. - topk) * n_fts, min=0, max=n_fts - 1).long() # Might just be for CV but doesn't matter since we do not use L1-norm
